...
/*   Updated: 2025/03/20 21:34:11 by rick             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_input_file(t_redirection *redir, t_command *command, t_data *data)
{
	...
	if (dup2(command->fd_in, STDIN_FILENO) == -1)
		return (close(command->fd_in), command->fd_in = -1, 1);
	return (0);
}
...
int	handle_heredoc(t_redirection *redir, t_command *command, t_data *data)
{
	...
	else
	{
		...
		command->fd_in = fd[0];
		if (dup2(command->fd_in, STDIN_FILENO) == -1)
			return (close_both(command->fd_in, std_in), 1);
		return (update_exit_status(pid, fd, data, std_in), 0);
	}
	return (0);
}
